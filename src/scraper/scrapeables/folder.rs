...
impl Scrapeable for IliasFolder {
    async fn scrape(
        &self,
        client: &Client,
        ...
        // Optional: just log the parser warnings (not critical)
        ...
        // extract links
        let link_selector = scraper::Selector::parse("a.il_ContainerItemTitle").unwrap();
        ...
        let links: Vec<Box<dyn Scrapeable + Send + Sync>> = document
            ...
            .enumerate()
            .filter_map(|(idx, link)| {
                let child_url = link.value().attr("href").map(|s| s.to_string())?;
                let child_name = sanitize_name(link.inner_html());
